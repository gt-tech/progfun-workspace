...
abstract class TweetSet {
  ...
  // helper method added to solve for mostRetweeted
  def empty: Boolean
...
class Empty extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = acc // it's empty so return accumulator as-is

  def descendingByRetweet: objsets.TweetList = Nil // empty list as no tweets to sort
  def mostRetweeted: objsets.Tweet = throw new NoSuchElementException("Empty.mostRetweeted")
  def union(that: objsets.TweetSet): objsets.TweetSet = that // nothing to union, return that as-is
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    // process all on the right and then move to process on the left
    // TODO: Interesting to know the processing ordering of this though
    // or expression application.
  ...
  def mostRetweeted: objsets.Tweet = {
  ...
  def union(that: objsets.TweetSet): objsets.TweetSet = {
