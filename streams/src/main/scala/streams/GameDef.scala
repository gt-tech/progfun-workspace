...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def neighbors: List[(Block, Move)] = List(left -> Left, right -> Right, up -> Up, down -> Down) // or (left, Left) :: (right, Right) :: (up, Up) :: (down, Down) :: Nil
     ...
     */
    def legalNeighbors: List[(Block, Move)] = for { n <- neighbors; if (n._1.isLegal) } yield n // iterate thru neighbors and filter out neigbors saying they illegal

    /**
     * Returns `true` if the block is standing.
     */
    def isStanding: Boolean = {
      b1.y == b2.y && b1.x == b2.x // assuming a 2-D terrain, both the boxes should be residing in a single cell (stacked one over other) 
    }
