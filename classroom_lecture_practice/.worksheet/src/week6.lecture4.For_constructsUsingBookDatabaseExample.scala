...
object For_constructsUsingBookDatabaseExample {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(116); 
  ...
  for (b <- getAllBooks(); a <- b.authors if a.contains("Doe")) yield b;System.out.println("""res0: List[week6.lecture4.Book] = """ + $show(res$0));$skip(316); val res$1 = 
  ...
  getAllBooks().flatMap { b => b.authors.withFilter(a => a.contains("Doe")) map (y => { println("y => " + y); b }) };System.out.println("""res1: List[week6.lecture4.Book] = """ + $show(res$1));$skip(116); val res$2 = 

  // find all books whose name/title contains Scala
  for (b <- getAllBooks() if b.name contains "Scala") yield b;System.out.println("""res2: List[week6.lecture4.Book] = """ + $show(res$2));$skip(1035); 
  ...
  }.distinct;System.out.println("""a  : List[String] = """ + $show(a ));$skip(15);  // distinct to make sure author names are unique and not duplicated
