...
object For_constructsUsingBookDatabaseExample {
  ...
  for (b <- getAllBooks() if b.name contains "Scala") yield b
                                                  //> res1: List[week6.lecture4.Book] = List(Book(Programming in Scala,List(John D
                                                  //| oe, Martin Oderskey)), Book(Scala - In depth,List(John Doe)))
  ...
  }.distinct                                      //> a  : List[String] = List(John Doe)
